...
class RequestPreset(Standard):
	...
	def start(self):
		...
		self.preset_data = []
		self.num_received_1f = 0
		next_packet_double = self.helix_usb.next_preset_data_packet_double()

		data = [0x19, 0x0, 0x0, 0x18, 0x80, 0x10, 0xed, 0x3, 0x0, "XX", 0x0, 0xc, self.helix_usb.maybe_session_no, next_packet_double[0], next_packet_double[1], 0x0, 0x1, 0x0, 0x6, 0x0, 0x9, 0x0, 0x0, 0x0, 0x83, 0x66, 0xcd, 0x3, 0xeb, 0x64, 0x16, 0x65, 0xc0, 0x0, 0x0, 0x0]
	...
	def data_in(self, data_in):
		...
		if self.helix_usb.my_byte_cmp(left=data_in, right=["XX", "XX", 0x0, 0x18, 0xed, 0x3, 0x80, 0x10, 0x0, "XX", 0x0, 0x4, "XX", "XX", 0x0, 0x0], length=16):
			...
			if data_in[1] == 0x0:
				...
				# ToDo: This is a beast: we need at least a double, maybe a tripe for the preset_data_packet_no
				data_out = [0x8, 0x0, 0x0, 0x18, 0x80, 0x10, 0xed, 0x3, 0x0, "XX", 0x0, 0x8, self.helix_usb.maybe_session_no, self.helix_usb.preset_data_packet_double(), 0x0, 0x0]
